%% @doc Integration tests for ClickHouse Array type support
%%
%% This module contains integration tests that verify array encoding/decoding
%% with a real ClickHouse server instance.
-module(clickhouse_erl_types_array_integration_tests).
...
%% @doc Test simple array round trip with ClickHouse
simple_array_roundtrip_test() ->
    test_helpers:setup(),
    ...
    ?assertMatch(#{data := #{rows := [[1, [1, 2, 3]], [2, [4, 5]], [3, [6]]]}}, Result),
...
%% @doc Test array of strings
array_string_roundtrip_test() ->
    test_helpers:setup(),
    {ok, Conn} = test_helpers:connect(),

    % Create table with array of strings
    ...
    % Verify results
    ?assertMatch(
        #{data := #{rows := [[1, [<<"tag1">>, <<"tag2">>]], [2, [<<"tag3">>]], [3, []]]}},
        Result
    ),
...
%% @doc Test nested arrays
nested_array_roundtrip_test() ->
    test_helpers:setup(),
    {ok, Conn} = test_helpers:connect(),

    % Create table with nested array
...
%% @doc Test array of tuples
array_of_tuples_roundtrip_test() ->
    test_helpers:setup(),
    {ok, Conn} = test_helpers:connect(),

    % Create table with array of tuples
    ...
    % Verify results
    ?assertMatch(
        #{data := #{rows := [[1, [{10, 20}, {30, 40}]], [2, [{50, 60}]]]}},
        Result
    ),
...
empty_array_roundtrip_test() ->
    ...
    % Verify results
    ?assertMatch(#{data := #{rows := [[1, []], [2, []], [3, []]]}}, Result),
