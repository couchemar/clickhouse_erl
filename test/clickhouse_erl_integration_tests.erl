%% @doc Integration tests for ClickHouse connection handshake
%%
%% This module contains integration tests that verify the complete handshake
%% flow with a real ClickHouse server instance, concurrent connections,
%% and error scenarios with network interruptions.
%%
%% Requirements: 1.1, 2.1, 3.1
-module(clickhouse_erl_integration_tests).
...
%% Requirements: 1.1, 2.1, 3.1
successful_handshake_default_test() ->
    test_helpers:setup(),

    % Test connection with Docker ClickHouse credentials
    Options = #{
    ...
    case Result of
        ...
        {error, Reason} ->
            ...
            ?debugFmt("Connection failed: ~p~n", [Reason]),
            ?assert(false)
    end,
...
%% Requirements: 1.1, 2.1, 3.1
successful_handshake_custom_credentials_test() ->
    test_helpers:setup(),
    ...
    case Result of
        ...
        {error, Reason} ->
            test_helpers:cleanup(),
            ?debugFmt("Connection with custom credentials failed: ~p~n", [Reason]),
            ?assert(false)
    end,
...
handshake_different_client_versions_test() ->
    test_helpers:setup(),
...
%% Requirements: 2.1, 3.1
handshake_different_databases_test() ->
    test_helpers:setup(),
...
%% Requirements: 1.1, 2.1, 3.1
concurrent_different_credentials_test() ->
    test_helpers:setup(),
...
%% Requirements: 1.1
connection_nonexistent_host_test() ->
    test_helpers:setup(),
...
%% Requirements: 1.1
resource_cleanup_after_failure_test() ->
    test_helpers:setup(),
...
exception_handling_authentication_failure_test() ->
    test_helpers:setup(),
...
exception_handling_connection_phases_test() ->
    test_helpers:setup(),
...
nested_exception_handling_test() ->
    test_helpers:setup(),
...
%% Requirements: 1.1, 2.1, 3.1, 6.1
exception_handling_error_scenarios_test() ->
    test_helpers:setup(),
...
exception_propagation_completeness_test() ->
    test_helpers:setup(),
...
roundtrip_bool_type_test() ->
    ...
    % Verify we got the correct data back
    % Note: Bool values are returned as 0/1 (UInt8)
    ?assertMatch(#{data := #{rows := [[1, 1], [2, 0], [3, 1], [4, 0]]}}, SelectResult),
...
insert_constraint_violation_test() ->
    test_helpers:setup(),
...
insert_row_count_mismatch_test() ->
    test_helpers:setup(),
...
insert_invalid_column_name_test() ->
    test_helpers:setup(),
...
new_bool_type_full_test() ->
    ...
    % Note: Bool values are returned as 0/1 (UInt8)
    ?assertMatch(
        #{data := #{rows := [[1, 1], [2, 0], [3, 1], [4, 0], [5, 1], [6, 0]]}},
        SelectResult
    ),
...
integration_test_() ->
    % 5 minute timeout for all integration tests
    {timeout, 300, [
        {"Complete Handshake Flow Tests", [
            fun successful_handshake_default_test/0,
            fun successful_handshake_custom_credentials_test/0,
            fun handshake_different_client_versions_test/0,
            fun handshake_different_databases_test/0
        ]},
        {"Concurrent Connections Tests", [
            fun concurrent_connections_test/0,
            fun connection_pooling_test/0,
            fun concurrent_different_credentials_test/0
        ]},
        {"Error Scenarios Tests", [
            fun connection_nonexistent_host_test/0,
            fun connection_invalid_port_test/0,
            fun connection_timeout_test/0,
            fun invalid_credentials_test/0,
            fun malformed_options_test/0,
            fun network_interruption_test/0,
            fun resource_cleanup_after_failure_test/0,
            fun connection_recovery_test/0
        ]},
        {"Stress Tests", [
            fun stress_rapid_connections_test/0
        ]},
        {"Exception Handling Tests", [
            fun exception_handling_authentication_failure_test/0,
            fun exception_handling_connection_phases_test/0,
            fun nested_exception_handling_test/0,
            fun exception_handling_error_scenarios_test/0,
            fun exception_propagation_completeness_test/0,
            fun exception_handling_integration_test/0,
            fun exception_packet_recognition_test/0
        ]},
        {"Error Information Tests", [
            fun error_information_completeness_test/0
        ]},
        {"Version Compatibility Tests", [
            fun version_compatibility_test/0
        ]},
        {"INSERT Query Integration Tests", [
            fun insert_single_column_test/0,
            fun insert_multiple_columns_test/0,
            fun insert_integer_types_test/0,
            fun insert_float_types_test/0,
            fun insert_string_type_test/0,
            fun insert_temporal_types_test/0,
            fun insert_bool_type_test/0,
            fun insert_datetime64_type_test/0,
            fun insert_empty_input_test/0,
            fun insert_schema_mismatch_test/0,
            fun insert_constraint_violation_test/0,
            fun insert_row_count_mismatch_test/0,
            fun insert_invalid_column_name_test/0,
            fun insert_large_dataset_test/0,
            fun insert_sequential_test/0,
            fun insert_with_timeout_test/0,
            fun insert_atomicity_test/0
        ]},
        {"Column Type Round-Trip Tests", [
            fun roundtrip_integer_types_test/0,
            fun roundtrip_float_types_test/0,
            fun roundtrip_temporal_types_test/0,
            fun roundtrip_bool_type_test/0
        ]},
        {"New Column Types Integration Tests (Task 8.1)", [
            fun new_integer_types_full_test/0,
            fun new_float_types_full_test/0,
            fun new_temporal_types_full_test/0,
            fun new_bool_type_full_test/0,
            fun new_all_types_combined_test/0
        ]}
    ]}.
