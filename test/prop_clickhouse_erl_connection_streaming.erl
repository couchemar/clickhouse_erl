...
prop_batch_mode_backward_compatibility() ->
    ?FORALL(
        NumDataBlocks,
        choose(1, 10),
        begin
            ...
            %% Simulate batch mode by using the default accumulation callback
            %% This is what happens when no on_data callback is provided
            DefaultCallback = fun(DataBlock, Acc) ->
                clickhouse_erl_response_handler:accumulate_data_block_callback(DataBlock, Acc)
            end,

            %% Process data blocks with default callback (batch mode)
            FinalResult = process_data_blocks_with_callback_and_initial_acc(
                DataBlocks,
                DefaultCallback,
                undefined
            ...
            case FinalResult of
                {ok, FinalAcc} ->
                    %% Verify the accumulator is a result_accumulator record
                    IsRecord =
                        is_tuple(FinalAcc) andalso
                            tuple_size(FinalAcc) =:= 5 andalso
                            element(1, FinalAcc) =:= result_accumulator,

                    %% Property 1: Result should be a result_accumulator record
                    case IsRecord of
                        true ->
                            %% Extract fields from result_accumulator
                            Columns = element(2, FinalAcc),
                            Rows = element(3, FinalAcc),
                            TotalRows = element(4, FinalAcc),
                            Statistics = element(5, FinalAcc),

                            %% Property 2: Columns should be a list
                            ColumnsIsList = is_list(Columns),

                            %% Property 3: Rows should be a list
                            RowsIsList = is_list(Rows),

                            %% Property 4: TotalRows should match the number of rows accumulated
                            TotalRowsCorrect = is_integer(TotalRows) andalso TotalRows >= 0,

                            %% Property 5: Statistics should be a map
                            StatisticsIsMap = is_map(Statistics),

                            %% Property 6: All data blocks should be accumulated
                            %% (we can't verify exact content without knowing the structure,
                            %% but we can verify the accumulator was used)
                            AccumulatorUsed = ColumnsIsList andalso RowsIsList,

                            ColumnsIsList andalso RowsIsList andalso TotalRowsCorrect andalso
                                StatisticsIsMap andalso AccumulatorUsed;
                        false ->
                            false
                    end;
                {error, _} ->
...
%% @doc Property 5: Callback Error Propagation
%% **Feature: streaming-query-results, Property 5: Callback Error Propagation**
%% **Validates: Requirements 1.4, 5.3**
%%
%% For any callback that returns `{error, Reason}`, the query should terminate immediately
%% and return `{error, {callback_failed, Reason}}` to the caller, without invoking further callbacks.
...
prop_optional_callback_invocation() ->
    ?FORALL(
        {NumProgressPackets, NumProfilePackets, NumProfileEventsPackets},
        {choose(0, 5), choose(0, 5), choose(0, 5)},
        begin
            %% Track callback invocations for each packet type
            put(progress_invocations, []),
            put(profile_invocations, []),
            put(profile_events_invocations, []),

            %% Create callbacks that record each invocation
            OnProgress = fun(ProgressInfo) ->
                Invocations = get(progress_invocations),
                put(progress_invocations, Invocations ++ [ProgressInfo]),
                ok
            end,

            OnProfile = fun(ProfileInfo) ->
                Invocations = get(profile_invocations),
                put(profile_invocations, Invocations ++ [ProfileInfo]),
                ok
            end,

            OnProfileEvents = fun(ProfileEventsInfo) ->
                Invocations = get(profile_events_invocations),
                put(profile_events_invocations, Invocations ++ [ProfileEventsInfo]),
                ok
            end,

            %% Generate random packet data for each type
            ProgressPackets = generate_progress_packets(NumProgressPackets),
            ProfilePackets = generate_profile_packets(NumProfilePackets),
            ProfileEventsPackets = generate_profile_events_packets(NumProfileEventsPackets),

            %% Simulate processing each packet type through its callback
            ProgressResult = process_optional_packets(ProgressPackets, OnProgress),
            ProfileResult = process_optional_packets(ProfilePackets, OnProfile),
            ProfileEventsResult = process_optional_packets(ProfileEventsPackets, OnProfileEvents),

            %% Verify the property: callbacks were invoked for each packet
            ProgressInvocations = get(progress_invocations),
            ProfileInvocations = get(profile_invocations),
            ProfileEventsInvocations = get(profile_events_invocations),

            %% Property 1: Number of progress callback invocations should equal number of progress packets
            ProgressCountMatches = (length(ProgressInvocations) =:= NumProgressPackets),

            %% Property 2: Number of profile callback invocations should equal number of profile packets
            ProfileCountMatches = (length(ProfileInvocations) =:= NumProfilePackets),

            %% Property 3: Number of profile_events callback invocations should equal number of profile_events packets
            ProfileEventsCountMatches =
                (length(ProfileEventsInvocations) =:= NumProfileEventsPackets),

            %% Property 4: Each progress packet should have triggered exactly one callback
            AllProgressPacketsProcessed = lists:all(
                fun(Packet) ->
                    lists:member(Packet, ProgressInvocations)
                end,
                ProgressPackets
            ),

            %% Property 5: Each profile packet should have triggered exactly one callback
            AllProfilePacketsProcessed = lists:all(
                fun(Packet) ->
                    lists:member(Packet, ProfileInvocations)
                end,
                ProfilePackets
            ),

            %% Property 6: Each profile_events packet should have triggered exactly one callback
            AllProfileEventsPacketsProcessed = lists:all(
                fun(Packet) ->
                    lists:member(Packet, ProfileEventsInvocations)
                end,
                ProfileEventsPackets
            ),

            %% Property 7: All processing should succeed (callbacks are non-fatal)
            AllSucceeded =
                (ProgressResult =:= ok) andalso (ProfileResult =:= ok) andalso
                    (ProfileEventsResult =:= ok),

            %% Property 8: Callbacks should be invoked even if they return ok (not {ok, _})
            %% This is different from on_data callbacks which must return {ok, NewAcc}
            CallbacksInvokedCorrectly =
                ProgressCountMatches andalso ProfileCountMatches andalso
                    ProfileEventsCountMatches,

            ProgressCountMatches andalso ProfileCountMatches andalso
                ProfileEventsCountMatches andalso AllProgressPacketsProcessed andalso
                AllProfilePacketsProcessed andalso AllProfileEventsPacketsProcessed andalso
                AllSucceeded andalso CallbacksInvokedCorrectly
        end
    ).

%% @doc Property 15: Batch Result Format
%% **Feature: streaming-query-results, Property 15: Batch Result Format**
%% **Validates: Requirements 9.2, 9.4 (with unified `data` key)**
%%
%% For any query without `on_data` callback that completes successfully, the result should be
%% a map containing `data` key with `#{columns => [...], rows => [...]}` structure and `statistics` key.

prop_batch_result_format() ->
    ?FORALL(
        NumDataBlocks,
        choose(1, 10),
        begin
            ...
            case FinalResult of
                {ok, ResultAccumulator} ->
                    %% Verify the accumulator is a result_accumulator record
                    IsRecord =
                        is_tuple(ResultAccumulator) andalso
                            tuple_size(ResultAccumulator) =:= 5 andalso
                            element(1, ResultAccumulator) =:= result_accumulator,

                    case IsRecord of
                        true ->
                            %% Extract fields from result_accumulator
                            Columns = element(2, ResultAccumulator),
                            Rows = element(3, ResultAccumulator),
                            _TotalRows = element(4, ResultAccumulator),
                            Statistics = element(5, ResultAccumulator),

                            %% Simulate the result format that would be returned by the connection
                            %% In batch mode, the result should have:
                            %% - data key with #{columns => [...], rows => [...]}
                            %% - statistics key with query statistics
                            SimulatedResult = #{
                                data => #{
                                    columns => Columns,
                            ...
                            %% Property 4: The 'data' value should be a map
                            DataValue = maps:get(data, SimulatedResult, undefined),
                            DataIsMap = is_map(DataValue),

                            %% Property 5: The 'data' map should contain 'columns' key
                            HasColumnsKey = maps:is_key(columns, DataValue),

                            %% Property 6: The 'data' map should contain 'rows' key
                            HasRowsKey = maps:is_key(rows, DataValue),

                            %% Property 7: The 'columns' value should be a list
                            ColumnsValue = maps:get(columns, DataValue, undefined),
                            ColumnsIsList = is_list(ColumnsValue),

                            %% Property 8: The 'rows' value should be a list
                            RowsValue = maps:get(rows, DataValue, undefined),
                            RowsIsList = is_list(RowsValue),

                            %% Property 9: The 'statistics' value should be a map
                            StatisticsValue = maps:get(statistics, SimulatedResult, undefined),
                            StatisticsIsMap = is_map(StatisticsValue),

                            %% Property 10: Result should NOT contain 'accumulator' key
                            %% (that's for streaming mode only)
                            NoAccumulatorKey = not maps:is_key(accumulator, SimulatedResult),

                            %% Property 11: The data structure should be nested correctly
                            %% data => #{columns => [...], rows => [...]}
                            NestedStructureCorrect =
                                DataIsMap andalso HasColumnsKey andalso HasRowsKey andalso
                                    ColumnsIsList andalso RowsIsList,

                            IsMap andalso HasDataKey andalso HasStatisticsKey andalso
                                DataIsMap andalso HasColumnsKey andalso HasRowsKey andalso
                                ColumnsIsList andalso RowsIsList andalso StatisticsIsMap andalso
                                NoAccumulatorKey andalso NestedStructureCorrect;
                        false ->
                            false
                    end;
                {error, _} ->
...
%% @doc Generate random progress packets
generate_progress_packets(NumPackets) ->
    [generate_single_progress_packet(N) || N <- lists:seq(1, NumPackets)].

%% @doc Generate a single progress packet with random data
generate_single_progress_packet(PacketNum) ->
    #{
        packet_num => PacketNum,
        rows => rand:uniform(1000),
        bytes => rand:uniform(10000),
        total_rows => rand:uniform(10000)
    }.

%% @doc Generate random profile packets
generate_profile_packets(NumPackets) ->
    [generate_single_profile_packet(N) || N <- lists:seq(1, NumPackets)].

%% @doc Generate a single profile packet with random data
generate_single_profile_packet(PacketNum) ->
    #{
        packet_num => PacketNum,
        rows => rand:uniform(1000),
        blocks => rand:uniform(100),
        bytes => rand:uniform(10000),
        applied_limit => rand:uniform(2) - 1,
        % 0 or 1
        rows_before_limit => rand:uniform(1000),
        calc_rows_before_limit => rand:uniform(2) - 1
        % 0 or 1
    }.

%% @doc Generate random profile_events packets
generate_profile_events_packets(NumPackets) ->
    [generate_single_profile_events_packet(N) || N <- lists:seq(1, NumPackets)].

%% @doc Generate a single profile_events packet with random data
generate_single_profile_events_packet(PacketNum) ->
    NumEvents = rand:uniform(5),
    Events = [
        #{
            name => list_to_binary("Event_" ++ integer_to_list(N)),
            value => rand:uniform(1000)
        }
     || N <- lists:seq(1, NumEvents)
    ],
    #{
        packet_num => PacketNum,
        rows => NumEvents,
...
%% @doc Process optional packets through callback
%% Optional callbacks return ok (not {ok, _}), and errors are non-fatal
process_optional_packets([], _Callback) ->
    ok;
process_optional_packets([Packet | Rest], Callback) ->
    %% Invoke callback - errors are non-fatal, so we always continue
    case Callback(Packet) of
        ok ->
            process_optional_packets(Rest, Callback);
        {error, _Reason} ->
            %% Optional callback errors are non-fatal, continue processing
            process_optional_packets(Rest, Callback);
        _InvalidReturn ->
            %% Invalid returns are also non-fatal for optional callbacks
            process_optional_packets(Rest, Callback)
    end.

%% @doc Property 9: Mode Alternation
%% **Feature: streaming-query-results, Property 9: Mode Alternation**
%% **Validates: Requirements 3.4**
%%
%% For any connection, alternating between Streaming_Mode (with callback) and Batch_Mode
%% (without callback) queries should work correctly, with no state leakage between queries.

prop_mode_alternation() ->
    ?FORALL(
        NumAlternations,
        choose(2, 5),
        begin
            %% Track query executions
            put(query_executions, []),

            %% Simulate alternating between streaming and batch modes
            ...
            %% Default batch mode callback (accumulates data blocks)
            BatchCallback = fun(DataBlock, Acc) ->
                clickhouse_erl_response_handler:accumulate_data_block_callback(DataBlock, Acc)
            end,

            %% Execute alternating queries
            AlternationResult = execute_alternating_queries(
                NumAlternations,
                StreamingCallback,
                BatchCallback
            ),

            %% Verify the property: mode alternation works correctly
            QueryExecutions = get(query_executions),

            case AlternationResult of
                {ok, Results} ->
                    %% Property 1: Number of results should equal number of alternations
                    ResultCountCorrect = (length(Results) =:= NumAlternations),

                    %% Property 2: Each result should be valid for its mode
                    AllResultsValid = verify_alternating_results(Results),

                    %% Property 3: Streaming results should be integers (row counts)
                    %% Batch results should be result_accumulator records
                    ResultTypesCorrect = verify_result_types(Results),

                    %% Property 4: No state leakage - each query should be independent
                    %% Verify that streaming accumulator doesn't affect batch mode
                    %% and batch data doesn't affect streaming mode
                    NoStateLeakage = verify_no_state_leakage(Results),

                    %% Property 5: All queries should have been executed
                    AllQueriesExecuted = (length(QueryExecutions) =:= NumAlternations),

                    %% Property 6: Query modes should alternate correctly
                    ModesAlternateCorrectly = verify_mode_alternation_pattern(QueryExecutions),

                    ResultCountCorrect andalso AllResultsValid andalso ResultTypesCorrect andalso
                        NoStateLeakage andalso AllQueriesExecuted andalso ModesAlternateCorrectly;
                {error, _Reason} ->
                    false
            end
        end
...
%% @doc Property 12: Optional Callback Non-Fatal Errors
%% **Feature: streaming-query-results, Property 12: Optional Callback Non-Fatal Errors**
%% **Validates: Requirements 7.4**
%%
%% For any optional callback (`on_progress`, `on_profile`, `on_profile_events`) that returns
%% `{error, Reason}`, the query should continue processing normally without terminating,
%% treating the error as non-fatal.

%% @doc Execute alternating streaming and batch queries
%% Returns {ok, Results} where Results is a list of query results
execute_alternating_queries(NumAlternations, StreamingCallback, BatchCallback) ->
    execute_alternating_queries(NumAlternations, StreamingCallback, BatchCallback, [], 1).

execute_alternating_queries(0, _StreamingCallback, _BatchCallback, Results, _QueryNum) ->
    {ok, lists:reverse(Results)};
execute_alternating_queries(
    NumRemaining,
    StreamingCallback,
    BatchCallback,
    Results,
    ...
    %% Determine mode based on query number (odd = streaming, even = batch)
    IsStreaming = (QueryNum rem 2) =:= 1,

    %% Record query execution
    Executions = get(query_executions),
    Mode =
        case IsStreaming of
            true -> streaming;
            false -> batch
        end,
    put(query_executions, Executions ++ [{QueryNum, Mode}]),

    %% Generate data blocks for this query
    NumDataBlocks = rand:uniform(3) + 1,
    DataBlocks = generate_data_blocks(NumDataBlocks),
    ...
    Result =
        case IsStreaming of
            true ->
                %% Streaming mode: use streaming callback with initial accumulator 0
                process_data_blocks_with_callback_and_initial_acc(
                    DataBlocks,
                    StreamingCallback,
                    0
                );
            false ->
                %% Batch mode: use batch callback with undefined initial accumulator
                process_data_blocks_with_callback_and_initial_acc(
                    DataBlocks,
                    BatchCallback,
                    undefined
...
%% @doc Verify that all alternating results are valid
verify_alternating_results([]) ->
    true;
verify_alternating_results([{streaming, Result} | Rest]) ->
    %% Streaming result should be an integer (row count)
    case is_integer(Result) of
        true -> verify_alternating_results(Rest);
        false -> false
    end;
verify_alternating_results([{batch, Result} | Rest]) ->
    %% Batch result should be a result_accumulator record
    IsRecord =
        is_tuple(Result) andalso
            tuple_size(Result) =:= 5 andalso
            element(1, Result) =:= result_accumulator,
    case IsRecord of
        true -> verify_alternating_results(Rest);
        false -> false
    end.

%% @doc Verify that result types match their modes
verify_result_types([]) ->
    true;
verify_result_types([{streaming, Result} | Rest]) ->
    %% Streaming results should be integers
    case is_integer(Result) of
        true -> verify_result_types(Rest);
        false -> false
    end;
verify_result_types([{batch, Result} | Rest]) ->
    %% Batch results should be result_accumulator records
    IsRecord =
        is_tuple(Result) andalso
            tuple_size(Result) =:= 5 andalso
...
%% @doc Verify that there is no state leakage between queries
%% Each query should be independent - streaming accumulator shouldn't affect batch,
%% and batch data shouldn't affect streaming
verify_no_state_leakage([]) ->
    true;
verify_no_state_leakage([_SingleResult]) ->
    %% Only one result, no leakage possible
    true;
verify_no_state_leakage([{streaming, Result1}, {batch, Result2} | Rest]) ->
    %% Streaming result is an integer, batch result is a record
    %% Verify they are different types (no leakage)
    NoLeakage = is_integer(Result1) andalso is_tuple(Result2),
    case NoLeakage of
        true -> verify_no_state_leakage([{batch, Result2} | Rest]);
        false -> false
    end;
verify_no_state_leakage([{batch, Result1}, {streaming, Result2} | Rest]) ->
    %% Batch result is a record, streaming result is an integer
    %% Verify they are different types (no leakage)
    NoLeakage = is_tuple(Result1) andalso is_integer(Result2),
    case NoLeakage of
        true -> verify_no_state_leakage([{streaming, Result2} | Rest]);
        false -> false
    end;
verify_no_state_leakage([{streaming, _Result1}, {streaming, _Result2} | Rest]) ->
    %% Two streaming queries in a row - verify both are integers
    verify_no_state_leakage([{streaming, _Result2} | Rest]);
verify_no_state_leakage([{batch, _Result1}, {batch, _Result2} | Rest]) ->
    %% Two batch queries in a row - verify both are records
    verify_no_state_leakage([{batch, _Result2} | Rest]).

%% @doc Verify that query modes alternate correctly
verify_mode_alternation_pattern([]) ->
    true;
verify_mode_alternation_pattern([_SingleExecution]) ->
    %% Only one execution, pattern is trivially correct
    true;
verify_mode_alternation_pattern(Executions) ->
    %% Verify that modes alternate: streaming, batch, streaming, batch, ...
    ExpectedPattern = lists:map(
        fun(N) ->
            case (N rem 2) =:= 1 of
                true -> streaming;
                false -> batch
        ...
        lists:seq(1, length(Executions))
    ),
    ActualPattern = [Mode || {_QueryNum, Mode} <- Executions],
    ActualPattern =:= ExpectedPattern.

prop_optional_callback_non_fatal_errors() ->
    ?FORALL(
        {NumPackets, ErrorAtPacket, ErrorReason},
        {choose(3, 10), choose(1, 10), error_reason_gen()},
        begin
            %% Ensure ErrorAtPacket is within range
            ActualErrorPacket = min(ErrorAtPacket, NumPackets),

            %% Track callback invocations and errors
            put(callback_invocations, []),
            put(callback_errors, []),

            %% Create a callback that returns an error at a specific packet
            ErrorCallback = fun(PacketInfo) ->
                %% Record this invocation
                Invocations = get(callback_invocations),
                PacketNum = maps:get(packet_num, PacketInfo, 0),
                put(callback_invocations, Invocations ++ [PacketNum]),

                %% Return error at the specified packet
                case PacketNum of
                    ActualErrorPacket ->
                        %% Record the error
                        Errors = get(callback_errors),
                        put(callback_errors, Errors ++ [{PacketNum, ErrorReason}]),
                        {error, ErrorReason};
                    _ ->
                        ok
                end
            ...
            %% Generate random packets (using progress packets as example)
            Packets = generate_progress_packets(NumPackets),

            %% Process packets with error callback
            FinalResult = process_optional_packets(Packets, ErrorCallback),

            %% Verify the property: errors are non-fatal
            RecordedInvocations = get(callback_invocations),
            RecordedErrors = get(callback_errors),

            %% Property 1: Processing should complete successfully despite errors
            ProcessingCompleted = (FinalResult =:= ok),

            %% Property 2: ALL packets should be processed, including those after the error
            AllPacketsProcessed = (length(RecordedInvocations) =:= NumPackets),

            %% Property 3: The error should have been recorded
            ErrorRecorded = (length(RecordedErrors) =:= 1),

            %% Property 4: The error packet should match the expected packet
            ErrorPacketCorrect =
                case RecordedErrors of
                    [{PacketNum, Reason}] ->
                        (PacketNum =:= ActualErrorPacket) andalso (Reason =:= ErrorReason);
                    _ ->
                        false
                end,

            %% Property 5: Callbacks should continue after the error
            %% Verify that packets after the error packet were also processed
            PacketsAfterError = lists:filter(
                fun(PacketNum) -> PacketNum > ActualErrorPacket end,
                RecordedInvocations
            ),
            CallbacksContinuedAfterError =
                (length(PacketsAfterError) =:= (NumPackets - ActualErrorPacket)),

            %% Property 6: All packet numbers should be present in invocations
            ExpectedPacketNums = lists:seq(1, NumPackets),
            AllPacketNumsPresent = (RecordedInvocations =:= ExpectedPacketNums),

            %% Property 7: The error should not terminate the query
            %% (this is the core property - non-fatal errors)
            ErrorWasNonFatal = ProcessingCompleted andalso AllPacketsProcessed,

            ProcessingCompleted andalso AllPacketsProcessed andalso ErrorRecorded andalso
                ErrorPacketCorrect andalso CallbacksContinuedAfterError andalso
                AllPacketNumsPresent andalso ErrorWasNonFatal
        end
    ).

%% @doc Property 16: Callback Execution Context
%% **Feature: streaming-query-results, Property 16: Callback Execution Context**
%% **Validates: Requirements 10.1**
%%
%% For any callback invocation, the callback should execute synchronously within the connection
%% gen_server process (verifiable by comparing `self()` within callback to connection pid).
